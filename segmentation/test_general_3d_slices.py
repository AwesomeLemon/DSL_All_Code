    ...
    with torch.no_grad():
        for hosp, data_loader in data_loader_all_hospitals.items():
            ...
            for i, (data, target, misc) in enumerate(tqdm(data_loader)):
                ...
                if misc['patient_id'][0] == p:
                    pred_p.append(output)
                    label_p.append(target[None, ...])
                else:
                    if p is not None:
                        pred_p = torch.cat(pred_p, dim=0)
                        label_p = torch.cat(label_p, dim=0)
                        n_patients += 1
                        print(f"pred_p.shape {pred_p.shape} | label_p.shape {label_p.shape} | patient {p} | n_patients {n_patients}")
                        for i, metric in enumerate(metric_fns):
                            cur_metric = metric(pred_p, label_p, misc)
                            total_metrics[i] += cur_metric
                            print(f"{metric.__name__}: {cur_metric.item()}")
                    p = misc['patient_id'][0] # only one slice in the batch.
                    pred_p = [output]
                    label_p = [target[None, ...]]
            ...
            if len(pred_p) > 0:
                ...
                label_p = torch.cat(label_p, dim=0)
                for i, metric in enumerate(metric_fns):
                    total_metrics[i] += metric(pred_p, label_p, misc)
                n_patients += 1
                    ...
                    'test':
                        {
                            ...
                            **{met.__name__: {'avg': total_metrics[i].item() / n_patients} for i, met in enumerate(metric_fns)}
                        }
                }
                logger.info(to_save)
