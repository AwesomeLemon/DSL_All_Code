...
def get_transforms_G():
    ...
    transforms_args_test = {
        "CenterCrop": [256],
        "ToTensorScale": ['float', 255, 4], # maxv_lb == 4 because cervix has 4 non-background classes
        "NormalizeBoth": [0.5, 0.5]  # normalize both, like in heart & brain
    }
...
def partition_data(datadir, partition, siteA_dir, siteB_dir):
    ...
    data_dict = dict()
    # Also note that in other files it is actually train, not test => that's what I should do too.
    data_dict['test_all'] = os.path.join(datadir, siteA_dir, 'train_all.h5')
...
def load_partition_data_distributed_cervix(process_id, dataset, data_dir, partition_method,
    ...
    class_num = 2  # polyp or background
...
class DatasetD:
    ...
    def get_data(self, key_batch, transform_params):
        ...
        for key, trans_para in zip(key_batch, transform_params):
            ...
            # print(f"Train: datapoint['image'].shape {datapoint['image'].shape} | datapoint['mask'].shape {datapoint['mask'].shape}")
